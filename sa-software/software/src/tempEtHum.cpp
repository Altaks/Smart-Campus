...
void taskTempEtHum(void *pvParameters) {
  while(true){
    ...
    temperature = (isnan(TempEtHum.temperature) ?  NULL : round(TempEtHum.temperature * 10.0)/10.0); //arrondi la valeur et reste a NaN si NaN pour ne pas "arrondir" NaN
    humidite = (isnan(TempEtHum.humidity) ?  NULL : round(TempEtHum.humidity * 10.0)/10.0);
